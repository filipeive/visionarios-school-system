...
Route::middleware(['auth', 'verified'])->group(function () {
   ...
   // Gestão de Disciplinas
    ...
    Route::middleware('role:teacher')->prefix('teacher')->name('teacher.')->group(function () {
        ...
        Route::get('/class/{class}/grades', [TeacherPortalController::class, 'classGrades'])->name('class-grades');
   ...
   /*  Route::get('/search', function (Illuminate\Http\Request $request) {
        ...
        if (!$query || strlen($query) < 3) {
            return redirect()->back()->with('warning', 'Digite pelo menos 3 caracteres para pesquisar.');
        }
        ...
        if (auth()->user()->can('view_students')) {
            $results['students'] = App\Models\Student::where(function($q) use ($query) {
        ...
        if (auth()->user()->can('view_teachers')) {
            $results['teachers'] = App\Models\Teacher::where(function($q) use ($query) {
                $q->where('first_name', 'like', "%{$query}%")
                  ->orWhere('last_name', 'like', "%{$query}%")
        ...
        if (auth()->user()->can('view_classes')) {
            $results['classes'] = App\Models\ClassRoom::where('name', 'like', "%{$query}%")
                ->active()
                ->currentYear()
                ->limit(10)
                ->get();
